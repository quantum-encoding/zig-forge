...
///
/// Fixed pool of 64 inodes. Data page tables and children arrays
/// are dynamically allocated from PMM at runtime (not inline).
...
/// ARM64 uses identity mapping (phys == virt), no HHDM offset
const hhdm = struct {
    pub fn physToVirt(phys: u64) u64 {
        return phys; // identity map on ARM64
    }
};

const MAX_NODES: usize = 64;
const MAX_DATA_PAGES: usize = 256; // max pages per file (1MB), dynamically allocated
const MAX_CHILDREN: usize = 64; // max children per dir, dynamically allocated
const DATA_PAGES_PER_BLOCK: usize = types.PAGE_SIZE / @sizeOf(?types.PhysAddr); // 256
const CHILDREN_PER_BLOCK: usize = types.PAGE_SIZE / @sizeOf(usize); // 512

pub const RamfsNode = struct {
    name: [256]u8,
    name_len: u8,
    inode: vfs.Inode,
    /// Dynamically allocated page holding data page addresses.
    data_pages_ptr: ?[*]?types.PhysAddr = null,
    data_pages_phys: types.PhysAddr = 0,
    size: u64,
    /// Dynamically allocated page holding child pointers.
    children_ptr: ?[*]?*RamfsNode = null,
    ...
    fn ensureDataPages(self: *RamfsNode) ?[*]?types.PhysAddr {
    ...
    fn ensureChildren(self: *RamfsNode) ?[*]?*RamfsNode {
        if (self.children_ptr) |p| return p;
        const phys = pmm.allocPage() orelse return null;
        const virt = hhdm.physToVirt(phys);
        const ptr: [*]?*RamfsNode = @ptrFromInt(virt);
...
pub fn create(parent: *vfs.Inode, name: []const u8, mode: u32) ?*vfs.Inode {
    ...
    const cp = parent_node.ensureChildren() orelse return null;
...
fn ramfsRename(old_parent: *vfs.Inode, old_name: []const u8, new_parent: *vfs.Inode, new_name: []const u8) bool {
    ...
    if (new_node.children_ptr) |new_cp| {
        for (0..new_node.child_count) |i| {
            if (new_cp[i]) |existing| {
                if (nameEq(existing.name[0..existing.name_len], new_name)) {
                    ...
                    new_cp[new_node.child_count - 1] = null;
...
fn ramfsRead(desc: *vfs.FileDescription, buf: [*]u8, count: usize) isize {
    ...
    while (read_total < to_read) {
        ...
        const dp_val = if (node.dataPages()) |dp| dp[@as(usize, @truncate(page_idx))] else null;
        if (dp_val) |phys| {
...
fn lookup(parent: *vfs.Inode, name: []const u8) ?*vfs.Inode {
    ...
    const cp = node.children_ptr orelse return null;
