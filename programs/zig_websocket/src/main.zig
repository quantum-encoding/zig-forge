//! zig_websocket CLI - WebSocket Protocol Demo and Echo Server
//!
//! Commands:
...
pub fn main(init: std.process.Init) !void {
    ...
    if (std.mem.eql(u8, command, "demo")) {
        ...
        if (args.len < 3) {
            try stdout.print("Usage: websocket-demo encode <message>\n", .{});
            try stdout.flush();
            return;
        }
        ...
        if (args.len < 3) {
            try stdout.print("Usage: websocket-demo decode <hex>\n", .{});
            try stdout.flush();
            return;
        }
        ...
        if (args.len < 3) {
            try stdout.print("Usage: websocket-demo handshake <key>\n", .{});
            try stdout.flush();
            return;
        }
...
fn printUsage(stdout: anytype) !void {
    try stdout.print(
        \\zig_websocket - WebSocket Protocol Library Demo
        \\
        \\Usage:
        \\  websocket-demo demo                    Run interactive demo
        \\  websocket-demo encode <message>        Encode message to frame
        \\  websocket-demo decode <hex>            Decode hex frame
        \\  websocket-demo handshake <key>         Generate Sec-WebSocket-Accept
        \\  websocket-demo echo                    Echo server demo
        \\
        \\Examples:
        \\  websocket-demo demo
        \\  websocket-demo encode "Hello World"
...
fn runDemo(allocator: std.mem.Allocator, stdout: anytype) !void {
    try stdout.print("\n╔══════════════════════════════════════════════════════════════╗\n", .{});
    try stdout.print("║         zig_websocket - WebSocket Protocol Demo              ║\n", .{});
    try stdout.print("╚══════════════════════════════════════════════════════════════╝\n\n", .{});
    ...
    var text_frame = try websocket.Frame.init(allocator, true, .text, message);
    ...
    var masked_frame = try websocket.Frame.initMasked(allocator, true, .text, client_msg);
    ...
    var ping_frame = try websocket.Frame.init(allocator, true, .ping, "PING");
    ...
    try close_payload.appendSlice(&std.mem.toBytes(@as(u16, 1000))); // Normal closure
    try close_payload.appendSlice("Goodbye");

    var close_frame = try websocket.Frame.init(allocator, true, .close, close_payload.items);
    ...
    const generated_accept = try websocket.Handshake.generateAccept(allocator, test_key);
    defer allocator.free(generated_accept);

    try stdout.print("Generated Sec-WebSocket-Accept: {s}\n", .{generated_accept});
    try stdout.print("Expected: {s}\n", .{expected_accept});

    const is_valid = try websocket.Handshake.validate(allocator, test_key, generated_accept);
    ...
    var conn = websocket.Connection.init(allocator, true);
    defer conn.deinit();
    ...
    var encode_frame = try websocket.Frame.init(allocator, true, .text, roundtrip_msg);
    defer encode_frame.deinit(allocator);

    const encoded = try encode_frame.toBytes(allocator);
    defer allocator.free(encoded);

    const decode_result = try websocket.Frame.fromBytes(allocator, encoded);
...
fn encodeMessage(allocator: std.mem.Allocator, stdout: anytype, message: []const u8) !void {
    try stdout.print("\n╔══════════════════════════════════════════════════════════════╗\n", .{});
    try stdout.print("║ WebSocket Frame Encoding                                     ║\n", .{});
    try stdout.print("╚══════════════════════════════════════════════════════════════╝\n\n", .{});

    try stdout.print("Message: \"{s}\"\n", .{message});
    try stdout.print("Length: {} bytes\n\n", .{message.len});

    var frame = try websocket.Frame.init(allocator, true, .text, message);
    ...
    // Decode the header
    const header_result = try websocket.FrameHeader.fromBytes(bytes);
...
fn decodeFrame(allocator: std.mem.Allocator, stdout: anytype, hex_str: []const u8) !void {
    try stdout.print("\n╔══════════════════════════════════════════════════════════════╗\n", .{});
    try stdout.print("║ WebSocket Frame Decoding                                     ║\n", .{});
    try stdout.print("╚══════════════════════════════════════════════════════════════╝\n\n", .{});
    ...
    const frame_result = websocket.Frame.fromBytes(allocator, frame_bytes.items) catch |err| {
...
fn generateHandshake(allocator: std.mem.Allocator, stdout: anytype, key: []const u8) !void {
    try stdout.print("\n╔══════════════════════════════════════════════════════════════╗\n", .{});
    try stdout.print("║ WebSocket Handshake Generation (RFC 6455 Section 1.2)        ║\n", .{});
    try stdout.print("╚══════════════════════════════════════════════════════════════╝\n\n", .{});
    ...
    const is_valid = try websocket.Handshake.validate(allocator, key, accept);
    try stdout.print("{s}\n", .{if (is_valid) "PASS ✓" else "FAIL ✗"});
}

fn runEchoDemo(allocator: std.mem.Allocator, stdout: anytype) !void {
    try stdout.print("\n╔══════════════════════════════════════════════════════════════╗\n", .{});
    try stdout.print("║         WebSocket Echo Server Demo (Frame Handling)          ║\n", .{});
    try stdout.print("╚══════════════════════════════════════════════════════════════╝\n\n", .{});

    try stdout.print("This demo shows how frames are processed in sequence:\n\n", .{});

    var conn = websocket.Connection.init(allocator, false); // Client connection
    defer conn.deinit();

    try stdout.print("Step 1: Send text message\n", .{});
    var msg1 = try websocket.Frame.init(allocator, true, .text, "Hello");
    ...
    var ping = try websocket.Frame.init(allocator, true, .ping, "");
    ...
    var close = try websocket.Frame.init(allocator, true, .close, close_payload.items);
    defer close.deinit(allocator);
    try conn.processFrame(&close);
