//! AVX2 8-way Parallel SHA-256d with Midstate Optimization
//!
//! Uses pre-computed midstate to skip Block 1 processing.
//! Each batch hashes 8 different nonces in parallel.
...
const DOUBLE_HASH_PADDING: [8]Vec8u32 = .{
...
/// SHA-256 compression (8-way parallel)
fn sha256_compress(h: *[8]Vec8u32, w: *const [64]Vec8u32) void {
    var a = h[0];
    var b = h[1];
    var c = h[2];
    var d = h[3];
...
fn prepare_schedule(block: *const [16]Vec8u32, w: *[64]Vec8u32) void {
    comptime var i: usize = 0;
...
/// Store 8 hashes from vectorized state
fn store_hashes(h: *const [8]Vec8u32, hashes: *[8][32]u8) void {
    for (0..8) |word| {
...
/// Hash 8 nonces in parallel using precomputed midstate
pub fn hashBatchWithMidstate(
    midstate: *const midstate_scalar.Midstate,
    base_nonce: u32,
    hashes_out: *[8][32]u8,
) void {
    // Load midstate into vectors
    var state: [8]Vec8u32 = undefined;
    for (0..8) |i| {
        state[i] = @splat(midstate.state[i]);
    }

    // Prepare Block 2 with 8 different nonces
    var block2: [16]Vec8u32 = undefined;

    // Words 0-2: same for all lanes
    for (0..3) |word| {
        const offset = word * 4;
    ...
    var double_block: [16]Vec8u32 = undefined;

    // Words 0-7: The intermediate hash (already in vector form)
    inline for (0..8) |i| {
...
test "avx2 midstate matches scalar" {
    ...
    var simd_hashes: [8][32]u8 = undefined;
