...
// =============================================================================
// Behavioral Red Flags — "for now" tracker
// =============================================================================
//
// "For now" is the #1 shortcut phrase. When Claude says it, something got
// skipped. We track the last file being worked on (from Write/Edit markers)
// and send a CTKW (warn) packet so the receiver can build an audit table.

var last_file_buf: [256]u8 = undefined;
var last_file_len: usize = 0;

fn trackFileFromState(state: []const u8) void {
    // Look for "Write(filename)" or "Edit(filename)" patterns
    const prefixes = [_][]const u8{ "Write(", "Edit(", "Read(" };
    for (prefixes) |prefix| {
        if (std.mem.indexOf(u8, state, prefix)) |start| {
            const name_start = start + prefix.len;
            if (std.mem.indexOfPos(u8, state, name_start, ")")) |end| {
                const fname = state[name_start..end];
                if (fname.len > 0 and fname.len < last_file_buf.len) {
                    @memcpy(last_file_buf[0..fname.len], fname);
                    last_file_len = fname.len;
                }
                return;
            }
        }
...
fn getLastFile() []const u8 {
    if (last_file_len == 0) return "unknown";
    return last_file_buf[0..last_file_len];
}

// Case-insensitive scan for "for now" — fast bail on 'f' byte check first
fn hasForNow(buf: [*]const u8, len: usize) bool {
    if (len < 7) return false; // "for now" is 7 chars
    const data = buf[0..len];
    // Quick check: does 'f' or 'F' exist?
    var has_f = false;
    for (data) |c| {
        if (c == 'f' or c == 'F') {
            has_f = true;
    ...
    var i: usize = 0;
    while (i <= len - 7) : (i += 1) {
        if ((data[i] == 'f' or data[i] == 'F') and
            (data[i + 1] == 'o' or data[i + 1] == 'O') and
            (data[i + 2] == 'r' or data[i + 2] == 'R') and
            data[i + 3] == ' ' and
            (data[i + 4] == 'n' or data[i + 4] == 'N') and
            (data[i + 5] == 'o' or data[i + 5] == 'O') and
            (data[i + 6] == 'w' or data[i + 6] == 'W'))
        {
            // Verify it's a word boundary (not "inform now" — check before 'f')
            if (i > 0 and std.ascii.isAlphanumeric(data[i - 1])) {
                i += 7;
                continue;
            }
            return true;
...
fn extractForNowContext(buf: [*]const u8, len: usize) []const u8 {
    const data = buf[0..len];
    // Find "for now" position and grab surrounding context
    var i: usize = 0;
    while (i <= len - 7) : (i += 1) {
        if ((data[i] == 'f' or data[i] == 'F') and
            (data[i + 1] == 'o' or data[i + 1] == 'O') and
            ...
            (data[i + 6] == 'w' or data[i + 6] == 'W'))
        {
            ...
            const ctx_start = if (i > 40) i - 40 else 0;
            const ctx_end = @min(i + 47, len);
            return data[ctx_start..ctx_end];
        }
    }
    return data[0..@min(len, 80)];
}

fn ctkSendWarn(file: []const u8, context: []const u8, pid_hint: u32) void {
    if (ctk_socket < 0) return;

    // Packet format: "CTKW|<pid>|<file>|<context>\n"
    var pkt: [512]u8 = undefined;
    const header = std.fmt.bufPrint(&pkt, "CTKW|{d}|", .{pid_hint}) catch return;
    var pos = header.len;

    // Append file
    const file_copy = @min(file.len, 128);
    @memcpy(pkt[pos..][0..file_copy], file[0..file_copy]);
    pos += file_copy;
    pkt[pos] = '|';
    pos += 1;

    // Append context (sanitize: replace newlines/control chars with space)
    const ctx_copy = @min(context.len, pkt.len - pos - 2);
    for (context[0..ctx_copy], 0..) |c, j| {
        pkt[pos + j] = if (c < 0x20) ' ' else c;
    }
    pos += ctx_copy;
    pkt[pos] = '\n';
    pos += 1;

    _ = raw_sendto(ctk_socket, &pkt, pos, 0, @ptrCast(&ctk_addr), 16);
}
...
fn warden_write(fd: c_int, buf: [*]const u8, count: usize) callconv(.c) isize {
    ...
    if ((fd == 1 or fd == 2) and count > 3) {
        ...
        // Cognitive state markers (0xE2 lead byte = ⏺, ✻, spinners)
        if (hasPossibleMarker(buf, count)) {
            if (extractCognitiveState(buf, count)) |state| {
                ctkSendState(state, raw_getpid());
                trackFileFromState(state);
            }
        }

        // "For now" red flag detection — separate from marker check
        if (count >= 7 and hasForNow(buf, count)) {
            const ctx = extractForNowContext(buf, count);
            ctkSendWarn(getLastFile(), ctx, raw_getpid());
        }
    }
...
fn warden_writev(fd: c_int, iov: [*]const Iovec, iovcnt: c_int) callconv(.c) isize {
    ...
    if ((fd == 1 or fd == 2) and iovcnt > 0) {
        ...
        for (iov[0..cnt]) |vec| {
            if (vec.iov_len > 3) {
                // Cognitive markers
                if (hasPossibleMarker(vec.iov_base, vec.iov_len)) {
                ...
                // "For now" red flag
                if (vec.iov_len >= 7 and hasForNow(vec.iov_base, vec.iov_len)) {
                    const ctx = extractForNowContext(vec.iov_base, vec.iov_len);
                    ctkSendWarn(getLastFile(), ctx, raw_getpid());
                }
            }
        }
