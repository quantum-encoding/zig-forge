...
pub fn main(init: std.process.Init) !void {
    ...
    try stdout.print("\nzig_socket - Performance Benchmarks\n", .{});
    try stdout.print("═══════════════════════════════════\n\n", .{});
    ...
    try stdout.print("TCP Socket Benchmarks:\n", .{});
    try stdout.print("─────────────────────────────────────────────────────────────\n", .{});

    result = try benchmarkSocketCreation(arena, 10000);
    try printBenchmarkResult(stdout, result);
    arena.free(result.name);
...
fn printBenchmarkResult(stdout: anytype, result: BenchmarkResult) !void {
    ...
    try stdout.print("{d:>8.2} us/op, ", .{@as(f64, @floatFromInt(result.avg_ns)) / 1000.0});
...
fn benchmarkFrameEncoding(allocator: std.mem.Allocator, iterations: u32) !BenchmarkResult {
    ...
    while (i < iterations) : (i += 1) {
        var frame = try socket.Frame.init(allocator, true, .text, payload);
...
fn benchmarkMaskedFrameEncoding(allocator: std.mem.Allocator, iterations: u32) !BenchmarkResult {
    ...
    var i: u32 = 0;
    while (i < iterations) : (i += 1) {
        var frame = try socket.Frame.initMasked(allocator, true, .text, payload);
        const bytes = try frame.toBytes(allocator);
        allocator.free(bytes);
        frame.deinit(allocator);
    }
...
fn benchmarkFrameDecoding(allocator: std.mem.Allocator, iterations: u32) !BenchmarkResult {
    ...
    var i: u32 = 0;
    while (i < iterations) : (i += 1) {
        const result = try socket.Frame.fromBytes(allocator, bytes);
...
fn benchmarkHeaderParsing(allocator: std.mem.Allocator, iterations: u32) !BenchmarkResult {
    ...
    var i: u32 = 0;
    while (i < iterations) : (i += 1) {
        _ = try socket.FrameHeader.fromBytes(bytes);
    }
    const elapsed = timer.read();
...
fn benchmarkMasking(allocator: std.mem.Allocator, iterations: u32) !BenchmarkResult {
    ...
    var i: u32 = 0;
    while (i < iterations) : (i += 1) {
        const result = try socket.Frame.fromBytes(allocator, frame_bytes);
        allocator.free(result.frame.payload);
    }
    const elapsed = timer.read();
...
fn benchmarkSocketCreation(allocator: std.mem.Allocator, iterations: u32) !BenchmarkResult {
    var timer = try Timer.start();
    var i: u32 = 0;
    while (i < iterations) : (i += 1) {
    ...
    return BenchmarkResult{
        .name = try allocator.dupe(u8, "TCP socket create/close"),
        .iterations = iterations,
        .total_ns = elapsed,
        .avg_ns = elapsed / iterations,
        .ops_per_sec = @as(u64, @intCast(iterations)) * 1_000_000_000 / elapsed,
