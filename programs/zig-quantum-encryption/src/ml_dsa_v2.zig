...
pub const Poly = struct {
    ...
    pub fn pointwiseMul(result: *Poly, a: *const Poly, b: *const Poly) void {
        for (0..N) |i| {
            result.coeffs[i] = modReduce(@as(i64, a.coeffs[i]) * @as(i64, b.coeffs[i]));
        }
    }
...
/// where R = 2^32
/// NOTE: Not used - simple modReduce is more reliable for ML-DSA
pub fn montgomeryReduce(a: i64) i32 {
    const a_lo: i32 = @truncate(a);
    const t: i32 = a_lo *% @as(i32, @truncate(Q_INV));
    const result: i32 = @truncate((a - @as(i64, t) * Q) >> 32);
    return result;
}
    ...
    while (len >= 1) : (len /= 2) {
        ...
        while (start < N) : (start += 2 * len) {
            ...
            for (start..start + len) |j| {
                const t = modReduce(zeta * @as(i64, p.coeffs[j + len]));
...
pub fn nttInverse(p: *Poly) void {
    ...
    while (len < N) : (len *= 2) {
        ...
        while (start < N) : (start += 2 * len) {
            k -= 1;
            const zeta: i64 = @as(i64, Q - ZETAS[k]); // -zeta mod q

            for (start..start + len) |j| {
                const t = p.coeffs[j];
                p.coeffs[j] = t + p.coeffs[j + len];
                p.coeffs[j + len] = modReduce(zeta * @as(i64, t - p.coeffs[j + len]));
            }
        }
    }
    ...
    for (0..N) |j| {
        p.coeffs[j] = modReduce(n_inv * @as(i64, p.coeffs[j]));
    }
}
...
pub fn sampleInBall(c: *Poly, seed: *const [CTILDE_BYTES]u8) void {
    var xof = crypto.hash.sha3.Shake256.init(.{});
    xof.update(seed);
...
pub fn makeHint(z: i32, r: i32) u1 {
    _ = lowBits(r); // Required by FIPS 204 for side-channel resistance
    const r1 = highBits(r);
...
/// Algorithm 37 from FIPS 204
/// m = (q-1)/(2*γ2) = 16 for ML-DSA-65
pub fn useHint(h: u1, r: i32) i32 {
    const m = (Q - 1) / (2 * GAMMA2); // = 16 for ML-DSA-65
    const d = decompose(r);
    if (h == 0) return d.r1;
...
pub fn sign(sk: *const SecretKey, msg: []const u8, randomized: bool) ?Signature {
    ...
    while (kappa < 1000) : (kappa += 1) {
        ...
        for (0..L) |i| {
            const nonce: u16 = @intCast(kappa * L + i);
            sampleGamma1(&y.polys[i], &rhoprime, nonce);
        }
        ...
        // Make hints
        // CRITICAL: Must reduce (w - cs2 + ct0) mod q before hint computation
        // because verify's w_approx is reduced. Without this, hint values are
        // computed on different representations causing verification failure.
        var hints: [K][N]u1 = undefined;
        var hint_count: usize = 0;

        for (0..K) |i| {
            for (0..N) |j| {
                // Reduce to [0, q-1] to match verify's w_approx representation
                const w_minus_cs2_plus_ct0 = reduce32(
        ...
        // Pack c_tilde (48 bytes for ML-DSA-65)
        @memcpy(sig.data[0..CTILDE_BYTES], &c_tilde);

        // Pack z (20 bits per coeff, need to handle signed values)
        // First reduce z to centered form
        offset = CTILDE_BYTES;
        for (0..L) |i| {
            for (0..N) |j| {
...
pub fn verify(pk: *const PublicKey, msg: []const u8, sig: *const Signature) bool {
    ...
    // Unpack signature
    const c_tilde = sig.data[0..CTILDE_BYTES];
    ...
    for (0..K) |i| {
        for (0..N) |j| {
            t1_hat.polys[i].coeffs[j] = modReduce(@as(i64, t1_hat.polys[i].coeffs[j]) * (1 << D));
        }
    }
    ...
    var c_tilde_prime: [CTILDE_BYTES]u8 = undefined;
    var h3 = crypto.hash.sha3.Shake256.init(.{});
    h3.update(&mu);
    for (0..K) |i| {
        var w1_bytes: [192]u8 = undefined;
