...
fn main() {
    ...
    let lib_name = match (target_os.as_str(), target_arch.as_str()) {
        ("macos", "aarch64") => "quantum_vault_macos-arm64",
        ("macos", "x86_64") => "quantum_vault_macos-x86_64",
        ("linux", "x86_64") => "quantum_vault_linux-x86_64",
        ("windows", "x86_64") => "quantum_vault_windows-x86_64",
        ("ios", "aarch64") => "quantum_vault_ios-arm64",
        ("android", "aarch64") => "quantum_vault_android-arm64",
        ("android", "arm") => "quantum_vault_android-arm32",
        _ => "quantum_vault", // Default native library
    };
    ...
    if prebuilt_exists {
        ...
        let zig_out_dir = manifest_dir.parent()
    ...
    match target_os.as_str() {
        ...
        "linux" | "android" => {
            // No additional libraries needed (uses getrandom)
        }
        _ => {}
    }

    // Re-run if library files change
