...
// TLS + WebSocket connection to wss://api.x.ai/v1/realtime
// Adapted from lyria_ws.zig for xAI's auth model (Bearer token in HTTP header)
...
const XAI_WS_HOST = "api.x.ai";
const XAI_WS_PORT: u16 = 443;
const XAI_WS_PATH = "/v1/realtime";
...
/// TLS connection wrapper (same pattern as lyria_ws.zig)
    ...
    fn init(allocator: Allocator, stream: net.Stream, io: Io, host: []const u8) !*TlsConnection {
        ...
        conn.tls_client = tls.Client.init(
            ...
            .{
                ...
                .entropy = &entropy,
                .realtime_now = blk: {
                    const realtime: std.Io.Timestamp = .{
...
pub const XaiWsClient = struct {
    ...
    /// Connect to xAI Realtime WebSocket with Bearer token auth
    pub fn connect(self: *Self, api_key: []const u8) !void {
        const io = self.io_threaded.io();

        // DNS resolution
        var host_buf: [256]u8 = undefined;
        @memcpy(host_buf[0..XAI_WS_HOST.len], XAI_WS_HOST);
        host_buf[XAI_WS_HOST.len] = 0;
        ...
        // TLS handshake
        self.tls_connection = try TlsConnection.init(self.allocator, self.stream.?, io, XAI_WS_HOST);
        errdefer if (self.tls_connection) |tc| tc.deinit();

        // WebSocket upgrade with Bearer auth
    ...
    /// WebSocket upgrade handshake — auth via Authorization header (not query param)
    fn doHandshake(self: *Self, api_key: []const u8) !void {
        ...
        const request = try std.fmt.bufPrint(&request_buf,
            "GET {s} HTTP/1.1\r\n" ++
                ...
                "\r\n",
            .{ XAI_WS_PATH, XAI_WS_HOST, api_key, key },
        );
...
test "XaiWsClient init" {
    const allocator = std.testing.allocator;
    var client = try XaiWsClient.init(allocator);
    defer client.deinit();
}
