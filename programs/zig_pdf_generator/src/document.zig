...
const montserrat_metrics = @import("fonts/montserrat_metrics.zig");
...
pub const Font = enum {
    ...
    // Custom TrueType — metrics generated by tools/extract_font_metrics.py,
    // TTF bytes embedded at compile time and written into the PDF as
    // /FontFile2 streams with /FlateDecode compression.
    montserrat_regular,
    montserrat_bold,
    ...
    /// Returns true for fonts that must be embedded as /Subtype /TrueType
    /// with a full /FontDescriptor and /FontFile2. Standard 14 fonts return
    /// false — PDF viewers already know them.
    pub fn isEmbedded(self: Font) bool {
        return switch (self) {
            .montserrat_regular, .montserrat_bold => true,
            else => false,
        };
    }

    /// Returns the raw TTF bytes for embedded fonts, or null for standard fonts.
    /// The bytes are @embedFile'd, so this is a zero-cost lookup.
    pub fn fontFileBytes(self: Font) ?[]const u8 {
        return switch (self) {
            .montserrat_regular => @embedFile("fonts/Montserrat-Regular.ttf"),
            .montserrat_bold    => @embedFile("fonts/Montserrat-Bold.ttf"),
            else => null,
        };
    }

    /// Returns the precomputed PDF-units metrics for embedded fonts, or null
    /// for standard fonts (whose metrics are inline in this enum).
    pub fn embeddedMetrics(self: Font) ?*const montserrat_metrics.FontMetrics {
        return switch (self) {
            .montserrat_regular => &montserrat_metrics.regular,
            .montserrat_bold    => &montserrat_metrics.bold,
            else => null,
        };
    }
    ...
    pub fn avgCharWidth(self: Font) f32 {
        return switch (self) {
            ...
            .montserrat_regular => 0.55,
            .montserrat_bold => 0.60,
        };
    }
    ...
    pub fn charWidth(self: Font, char: u8) u16 {
        ...
        // Embedded TrueType fonts read from their generated metrics table.
        if (self.embeddedMetrics()) |m| return m.widths[char];
        ...
        return switch (self) {
            ...
            .montserrat_regular, .montserrat_bold => unreachable, // handled above
        };
    }
...
pub const PdfDocument = struct {
    ...
    pub fn build(self: *PdfDocument) ![]const u8 {
        ...
        // Reserve object IDs for fonts. Standard Type1 fonts take one object
        // (the font dict). Embedded TrueType fonts take three (dict + descriptor
        // + FontFile2 stream). Compute the dict object ID for each font up
        // front so the resources dictionary can reference them.
        const first_font_obj = self.next_object_id;
        var font_dict_ids: [MAX_FONTS]u32 = undefined;
        var font_total_objs: u32 = 0;
        {
            var cursor: u32 = first_font_obj;
            for (0..self.font_count) |i| {
                font_dict_ids[i] = cursor;
                cursor += if (self.font_ids[i].isEmbedded()) 3 else 1;
            }
            font_total_objs = cursor - first_font_obj;
        }
        self.next_object_id += font_total_objs;

        // Build font resource dictionary (/F0 N 0 R /F1 M 0 R …)
        ...
        for (0..self.font_count) |i| {
            var buf: [128]u8 = undefined;
            const len = std.fmt.bufPrint(&buf, "/F{d} {d} 0 R ", .{ i, font_dict_ids[i] }) catch continue;
        ...
        if (self.image_count > 0) {
            ...
            for (0..self.image_count) |i| {
                var buf: [128]u8 = undefined;
                const img_obj_id = first_image_obj + @as(u32, @intCast(i));
        ...
        // Write Font objects — standard fonts as a single Type1 dict,
        // embedded fonts as a three-object chain: TrueType dict, descriptor,
        // and FontFile2 stream.
        for (0..self.font_count) |i| {
            const font = self.font_ids[i];
            const dict_id = font_dict_ids[i];
            if (!font.isEmbedded()) {
                var buf: [256]u8 = undefined;
                const len = std.fmt.bufPrint(&buf, "<< /Type /Font /Subtype /Type1 /BaseFont /{s} /Encoding /WinAnsiEncoding >>", .{font.pdfName()}) catch continue;
                try self.writeObject(dict_id, len);
            } else {
                const descriptor_id = dict_id + 1;
                const fontfile_id = dict_id + 2;
                const m = font.embeddedMetrics().?;
                const ttf = font.fontFileBytes().?;

                // Widths array — 256 entries, space-separated.
                var widths_buf: std.ArrayListUnmanaged(u8) = .empty;
                defer widths_buf.deinit(self.allocator);
                try widths_buf.appendSlice(self.allocator, "[ ");
                for (m.widths) |w| {
                    var nbuf: [8]u8 = undefined;
                    const nlen = std.fmt.bufPrint(&nbuf, "{d} ", .{w}) catch continue;
                    try widths_buf.appendSlice(self.allocator, nlen);
                }
                try widths_buf.appendSlice(self.allocator, "]");

                // Font dictionary
                const font_dict_str = try std.fmt.allocPrint(self.allocator,
                    "<< /Type /Font /Subtype /TrueType /BaseFont /{s} /Encoding /WinAnsiEncoding /FirstChar 0 /LastChar 255 /Widths {s} /FontDescriptor {d} 0 R >>",
                    .{ font.pdfName(), widths_buf.items, descriptor_id },
                );
                defer self.allocator.free(font_dict_str);
                try self.writeObject(dict_id, font_dict_str);

                // Font descriptor
                const descriptor_str = try std.fmt.allocPrint(self.allocator,
                    "<< /Type /FontDescriptor /FontName /{s} /Flags {d} /FontBBox [{d} {d} {d} {d}] /ItalicAngle {d:.1} /Ascent {d} /Descent {d} /CapHeight {d} /XHeight {d} /StemV {d} /FontFile2 {d} 0 R >>",
                    .{
                        font.pdfName(), m.flags,
                        m.bbox_xmin, m.bbox_ymin, m.bbox_xmax, m.bbox_ymax,
                        m.italic_angle,
                        m.ascent, m.descent, m.cap_height, m.x_height, m.stem_v,
                        fontfile_id,
                    },
                );
                defer self.allocator.free(descriptor_str);
                try self.writeObject(descriptor_id, descriptor_str);

                // FontFile2 stream — the raw TTF, FlateDecode-compressed.
                // /Length1 is the uncompressed length (required by the spec
                // so decoders know how much TTF to expect after inflate).
                try self.writeFontFileObject(fontfile_id, ttf);
            }
        }
    ...
    /// Write a /FontFile2 stream containing a raw TTF. PDF requires
    /// /Length1 (the uncompressed TTF byte count) so viewers know how much
    ...
    fn writeFontFileObject(self: *PdfDocument, obj_id: u32, ttf: []const u8) !void {
        ...
        const compressed = deflateCompress(self.allocator, ttf) catch null;
        const use_compressed = compressed != null and compressed.?.len < ttf.len;
