...
pub const Reader = struct {
    ...
    fn restoreStatic(self: *Reader) void {
        ...
        if (start > self.static.len / 2) {
            ...
            // Performance optimization: Check next message length if available to
            // determine if compaction is necessary. This avoids unnecessary memory
            // operations when the remaining buffer space is sufficient.
            const pos = self.pos;
            const data_len = pos - start;
