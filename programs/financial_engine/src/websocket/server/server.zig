...
pub const Conn = struct {
    ...
    fn closeSocket(self: *Conn) void {
        if (@atomicRmw(bool, &self._closed, .Xchg, true, .monotonic) == false) {
            _ = std.c.close(self.stream.handle);
        }
    }
...
const global_test_allocator = std.heap.c_allocator;

test "tests:beforeAll" {
    test_server = try Server(TestHandler).init(global_test_allocator, .{
        .port = 9292,
        .address = "127.0.0.1",
    });
