...
test "seccomp: blocked syscall traps at runtime (Linux/x86_64)" {
    ...
    // Raw syscalls signal errors as -4095..-1 encoded in the returned usize.
    if (@as(isize, @bitCast(fork_rc)) < 0) return error.ForkFailed;
    ...
    try std.testing.expectEqual(std.os.linux.SIG.SYS, std.os.linux.W.TERMSIG(status));
}
