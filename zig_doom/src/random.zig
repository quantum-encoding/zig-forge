...
/// Debug: dump a stack trace on each pRandom call when set (sync hunting)
pub var debug_trace: bool = false;

/// P_Random — playsim random, deterministic for demo compatibility
pub fn pRandom() u8 {
    if (debug_trace) {
        std.debug.print("--- pRandom (idx {d} -> {d}) ---\n", .{ prnd_index, prnd_index +% 1 });
...
/// Current P_Random table index (sync tracing/debug)
pub fn getPrndIndex() u8 {
    return prnd_index;
}
